...
const DashboardPage = () => {
    ...
    useEffect(() => {
        ...
        const storedPartner = localStorage.getItem('zomatoPartner');
        if (!storedPartner) {
            navigate('/login');
            return;
        }
    ...
    const calculateStats = (data) => {
        const orders = data.reduce((acc, curr) => acc + curr.ordersAccepted, 0);
    ...
    const toggleStatus = (index) => {
        ...
        // If going offline, usually orders stop
        if (!newData[index].isOnline) {
            newData[index].ordersAccepted = 0;
    ...
    const updateOrders = (index, delta) => {
        const newData = [...hourlyData];
        // Can only add orders if online
        if (!newData[index].isOnline && delta > 0) {
            toast.error("Partner must be Online to accept orders!", {
                icon: '⚠️',
            });
            return;
        }

        const newCount = Math.max(0, newData[index].ordersAccepted + delta);
        newData[index].ordersAccepted = newCount;
    ...
    const handleLogout = () => {
        localStorage.removeItem('zomatoPartner');
    ...
    const handleSaveData = async () => {
        ...
        try {
            const response = await fetch('http://16.112.159.224:5000/api/partners/save-log', {
                method: 'POST',
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <header className="bg-white border-b border-gray-100 px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
                <div className="flex items-center gap-3">
                    <span className="text-2xl font-black italic text-red-500 tracking-tighter">zomato</span>
                ...
                <div className="flex items-center gap-6">
                    <button 
                        ...
                        className="hidden md:flex items-center gap-2 bg-red-500 text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-red-600 transition-all shadow-lg shadow-red-200 active:scale-95"
                    >
                        Sync Activity
                    </button>
                    ...
                    <button 
                        ...
                        className="p-3 rounded-full bg-gray-50 text-gray-500 hover:text-red-500 hover:bg-red-50 transition-all border border-gray-200 shadow-sm"
                    >
                        <FaSignOutAlt />
                    </button>
                </div>
            ...
            <main className="max-w-7xl w-full mx-auto px-6 py-10 flex-grow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 text-left">
                    <div className="bg-white p-6 rounded-3xl shadow-xl shadow-gray-100 border border-gray-100 flex items-center gap-5">
                        <div className="size-16 rounded-2xl bg-red-50 flex items-center justify-center text-red-500 text-3xl shrink-0 border border-red-100 transition hover:rotate-6">
                            <FaMotorcycle />
                        </div>
                        <div>
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Total Deliveries Today</p>
                            <h2 className="text-3xl font-black text-gray-900">{stats.totalOrders}</h2>
                        </div>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-white text-[11px] font-black tracking-[0.1em] text-gray-400 uppercase">
                                <tr>
                                    <th className="px-8 py-5 border-b border-gray-100">Time Slot</th>
                                    <th className="px-8 py-5 border-b border-gray-100">Toggle Status</th>
                                    <th className="px-8 py-5 border-b border-gray-100 text-center">Adjust Orders</th>
                                    <th className="px-8 py-5 border-b border-gray-100">Hourly Income</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={index} className="hover:bg-red-50/20 transition-colors group">
                                        ...
                                        <td className="px-8 py-5">
                                            <button 
                                                ...
                                                className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider border transition-all active:scale-95 ${row.isOnline ? 'bg-green-50 text-green-600 border-green-100 hover:bg-green-100' : 'bg-gray-100 text-gray-400 border-gray-200 hover:bg-gray-200'}`}
                                            >
                                        ...
                                        <td className="px-8 py-5">
                                            <div className="flex items-center justify-center gap-4">
                                                <button 
                                                    onClick={() => updateOrders(index, -1)}
                                                    className="size-8 rounded-lg bg-gray-100 text-gray-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all border border-gray-200 active:scale-90"
                                                >
                                                    <FaMinus size={10}/>
                                                </button>
                                                
                                                <div className="flex flex-col items-center min-w-[30px]">
                                                    <span className={`text-xl font-black ${row.ordersAccepted > 0 ? 'text-gray-900' : 'text-gray-300'}`}>
                                                        {row.ordersAccepted}
                                                    </span>
                                                </div>
                                        ...
                                        <td className="px-8 py-5">
                                            <span className={`text-base font-black ${row.earnings > 0 ? 'text-red-500' : 'text-gray-300'}`}>
                                                ₹{row.earnings}
                                            </span>
                                        </td>
                                    </tr>
