...
The ML Service is a standalone Python microservice responsible for three core intelligence capabilities of NichePay:
...
```bash
cd NichePay/Backend/ML-Service
python -m venv venv
...
**User-Agent:** `NichePay-ML-Service/1.0`
...
|---|---|---|
| `NEWS_API_KEY` | `497dfc77a3e0443d8c87e41a00f5e1be` | NewsAPI.org API key for fetching strike/curfew articles |

> ⚠️ **Security Note:** The default `NEWS_API_KEY` is embedded as a fallback for development. For production, always set this via environment variable or a secrets manager.
...
*NichePay ML Service — Built with FastAPI, scikit-learn, and real-time public APIs.*
